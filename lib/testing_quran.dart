...
class _TestingQuranState extends State<TestingQuran> {
        ...
        children: [
          ElevatedButton(
              onPressed: () async {
                ...
                // }
                await NotificationServices().scheduleNotification1(
                    id: 0, title: "title plan", body: "body plan", payload: "");
  ...
  Future<void> getQuran(BuildContext context) async {
    List<QuranText> quranText =
        await QuranDatabase().getQuranSurahText(surahId: 2);
    if (quranText != []) {
  ...
  Future<void> fetchData() async {
    final response =
        await http.get(Uri.parse("https://tanzil.net/trans/ur.maududi"));
    if (response.statusCode == 200) {
      var apiResponse = response.body;
      var list = apiResponse.split("\n");
      var updated = [];
      ...
      if (updated.length > 6000 && juzIds.length > 6000) {
        for (int i = 0; i < 6236; i++) {
          if (juzIds[i].juzId != null) {
            urdu.add(QuranTranslations(
                surahId: int.parse(updated[i][0]),
                verseId: int.parse(updated[i][1]),
                juzId: juzIds[i].juzId,
                verseText: updated[i][2]));
          }
        }
      }
      Hive.box(appBoxKey)
          .put("translation_ur_jalandhry", jsonEncode(urdu))
          .then((value) {
        print(QuranTranslations.fromJson(jsonDecode(
                Hive.box(appBoxKey).get("translation_ur_jalandhry"))[0])
            .verseText);
        debugPrint(
          Hive.box(appBoxKey).get("translation_ur_jalandhry")[0],
        );
