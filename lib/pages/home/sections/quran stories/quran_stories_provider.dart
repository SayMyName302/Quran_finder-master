...
class QuranStoriesProvider extends ChangeNotifier {
  ...
  Future<void> downloadStoryAudio(BuildContext context) async {
    try {
      ...
      Dio dio = Dio();
      var response = await dio.get(_selectedQuranStory!.audioUrl!,
      ...
      if (response.statusCode == 200) {
        ...
        String filePath =
            "$storiesAudioFolder/${_selectedQuranStory!.storyId!}.mp3";
  ...
  void gotoPlayerPage(BuildContext context, String audioFile) {
    Provider.of<StoryPlayerProvider>(context, listen: false).initAudioPlayer(
        audioFile, "assets/images/quran_stories/${selectedQuranStory!.image}");
