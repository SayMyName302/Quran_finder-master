...
class PrayerTimeProvider extends ChangeNotifier{
  ...
  final List<bool> _notificationState = Hive.box(appBoxKey).get(salahTimerListKey) ?? [false,false,false,false,false];
  ...
  Future<void> getSalahPageData(BuildContext context) async {
    ...
    try{
      ...
      List<Placemark?> placeMarks = await placemarkFromCoordinates(position.latitude, position.longitude,localeIdentifier: LocalizationProvider().locale.languageCode == "ur" || LocalizationProvider().locale.languageCode == "ar" ? LocalizationProvider().locale.toString() : "en");
      if(placeMarks.isNotEmpty){
        ...
        NetworksCheck(
          onComplete: (){
            Future.delayed(Duration.zero,()=>checkWeather(placeMark.locality!,context));
          },
          onError: (){
      ...
      EasyLoadingDialog.dismiss(context);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('please restart your location services or check network')));
    }catch (e){
      print(e);
      ...
      EasyLoadingDialog.dismiss(context);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please Go to Settings and allow application to use Your Location')));
    }
  }

  void setPrayerNotification(int index,){
  ...
  void getPrayerTime({required double lat,required double lang}) async {
    ...
    _prayerTimesList.add(Salah(name: "Fajar", time: DateFormat.jm().format(prayerTimes.fajr!).toString(), notify: _notificationState[0],prayerTime: prayerTimes.fajr!));
    _prayerTimesList.add(Salah(name: "dhuhar", time: DateFormat.jm().format(prayerTimes.dhuhr!).toString(), notify: _notificationState[1],prayerTime: prayerTimes.dhuhr!));
    _prayerTimesList.add(Salah(name: "Asr", time: DateFormat.jm().format(prayerTimes.asr!).toString(), notify: _notificationState[2],prayerTime: prayerTimes.asr!));
    _prayerTimesList.add(Salah(name: "Maghrib", time: DateFormat.jm().format(prayerTimes.maghrib!).toString(), notify: _notificationState[3],prayerTime: prayerTimes.maghrib!));
    _prayerTimesList.add(Salah(name: "Isha", time: DateFormat.jm().format(prayerTimes.isha!).toString(), notify: _notificationState[4],prayerTime: prayerTimes.isha!));
  ...
  void checkWeather(String city,BuildContext context) async {
    try {
      ...
      if(response.statusCode == 200){
        if(data['current']['condition']['text'].toString().toLowerCase().contains("rain")){
          image = 'assets/images/salah_timer/cloud_drizzle.png';
        }else if(data['current']['condition']['text'].toString().toLowerCase().contains("cloudy")){
          image = 'assets/images/salah_timer/cloud.png';
        }else if(data['current']['condition']['text'].toString().toLowerCase().contains("mist")){
          image = 'assets/images/salah_timer/cloud.png';
        }else if(data['current']['condition']['text'].toString().toLowerCase().contains("thunder")){
          image = 'assets/images/salah_timer/cloud_lightning.png';
        }else if(data['current']['condition']['text'].toString().toLowerCase().contains("snow")){
