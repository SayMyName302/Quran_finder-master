...
class RecitationCategoryProvider extends ChangeNotifier{
  ...
  gotoRecitationPlayerPage(
    ...
    if (_selectedRecitationAll.isNotEmpty) {
      _currentRecitationIndex = _selectedRecitationAll.indexWhere((element) => element.title == duaText);
  ...
  gotoRecitationAudioPlayerPage(int surahNo,imageUrl, BuildContext context, int index) {
    _currentRecitationIndex = _selectedRecitationAll.indexWhere((element) => element.surahNo == surahNo);
    _selectedRecitationStory = _selectedRecitationAll[_currentRecitationIndex];
    ...
    Provider.of<StoryAndBasicPlayerProvider>(context, listen: false)
        .initAudioPlayer(_selectedRecitationStory!.contentUrl!, "assets/images/recitation_category_images/${imageUrl!}",
        context);
   Navigator.of(context).pushNamed(RouteHelper.recitationAudioPlayer, arguments: 'fromRecitation');
  }
