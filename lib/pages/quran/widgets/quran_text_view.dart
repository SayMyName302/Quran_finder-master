...
class _QuranTextViewState extends State<QuranTextView> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero,()=>context.read<MyStateProvider>().startQuranReadingTimer("other"));
    ...
    _itemPositionsListener!.itemPositions.addListener(() {
      if(mounted){
        if(_itemPositionsListener!.itemPositions.value.isNotEmpty){
          _lastPosition = _itemPositionsListener!.itemPositions.value.first.index;
        }
      }
    });
    // _startTimer();
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: Consumer5<QuranProvider,ThemProvider,LocalizationProvider,FontProvider,AppColorsProvider>(
          builder: (context, value,them,language,font,appColors, child) {
            return value.quranTextList.isNotEmpty ? ScrollablePositionedList.builder(
              padding: EdgeInsets.only(bottom: 16.h),
              itemCount: value.quranTextList.length,
              itemScrollController: _itemScrollController,
              itemPositionsListener: _itemPositionsListener,
              // if 0 from home, 1 from surah or para, 2 from bookmark
              initialScrollIndex: _fromWhere == 0 ? Hive.box('myBox').get("lastSeen").lastSeen ?? 0 : _fromWhere == 1 ? 0 : _quranProvider.bookmarkPosition != -1 ? _quranProvider.bookmarkPosition : 0,
              itemBuilder: (context, index) {
                ...
                return Container(
                  margin: EdgeInsets.only(top: 13.h, left: 5.w, right: 5.w),
                  ...
                  child: Column(
                    ...
                    children: [
                      font.isQuranText ? Container(
                        margin: EdgeInsets.only(top: 16.h,left: 5.w,right: 5.w,),
                        child: Text(
                          quranText.verseText!,
                          textAlign: TextAlign.center,
                          style: TextStyle(
                      ...
                      font.isTranslationText ? Container(
                        margin: EdgeInsets.only(top: 10.h,left: 5.w,right: 5.w,bottom: 15.h),
                        child: Text(
                      ...
                      Container(
                        margin: EdgeInsets.only(left: 7.w, right: 13.w,bottom: 18.h),
                        decoration: BoxDecoration(
                            color: !them.isDark ? Colors.white : AppColors.grey2,
                        ...
                        child: Row(
                          ...
                          children: [
                            Container(
                              margin: EdgeInsets.only(top: 9.h,bottom: 10.h,left: 14.w,right: 23.w),
                              child: Text(
                                "${quranText.surahId}:${quranText.verseId}",
                                style: TextStyle(
                                    fontSize: 10.sp,
                                    fontFamily: 'satoshi',
                                    color: !them.isDark ? AppColors.grey1 : Colors.white,
                                    fontWeight: FontWeight.w700),
                              ),
                            ),
                            InkWell(
                              ...
                              child: Container(
                                height: 18.h,
                                width: 18.w,
                                margin: EdgeInsets.only(bottom: 7.h,top: 8.h,right: 20.w,left: 20.w),
                                child: CircleAvatar(
                                  ...
                                  child: SizedBox(
                                    ...
                                    width: 16.w,
                                    child: CircleAvatar(
                                      backgroundColor: !them.isDark ? quranText.isBookmark == 1 ? appColor : Colors.white : AppColors.grey2,
                                      child: Icon(
                                        Icons.bookmark,
                                        color: quranText.isBookmark == 1 ? Colors.white : appColors.mainBrandingColor,
                                        size: 9.h,
                                      ),
                                    ),
                                  ),
        ...
        bottomNavigationBar: Consumer2<AppColorsProvider,QuranProvider>(
          builder: (context, appColor,quranProvider, child) {
            return quranProvider.isJuz == false && quranProvider.nextSurah != null ?  Container(
              color: appColor.mainBrandingColor,
              height: 55.h,
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  Text('Next Surah',style: TextStyle(
                      color: Colors.white,
                      fontSize: 14.sp, fontFamily: 'satoshi',
                      fontWeight: FontWeight.w700
                  )),
                  Text('سورة${quranProvider.nextSurah!.arabicName}',style: TextStyle(
                      color: Colors.white,
                      fontSize: 24.sp, fontFamily: 'Al Majeed Quranic Font',
                      fontWeight: FontWeight.w700
                  )),
                  InkWell(
                    onTap: (){
                      context.read<QuranProvider>().setSurahText(surahId: _quranProvider.nextSurah!.surahId!,title: 'سورة ${_quranProvider.nextSurah!.arabicName}',fromWhere: 1);
                    ...
                    child: Container(
                      ...
                      child: Text('Continue to Next Surah',style: TextStyle(
                        color: appColor.mainBrandingColor,
                          fontSize: 12.sp, fontFamily: 'satoshi',
                          fontWeight: FontWeight.w700
                      ),),
                    ),
                  )
  ...
  Future<void> _showSettingDialog(BuildContext context) async {
    ...
    var style = TextStyle(fontSize: 14.sp, fontFamily: 'satoshi', fontWeight: FontWeight.w500);
    await showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          ...
          content: Consumer3<AppColorsProvider,FontProvider,TranslationManagerProvider>(
             ...
             return Column(
               ...
               children: [
                 ...
                 Row(
                   mainAxisAlignment: MainAxisAlignment.spaceBetween,
                   children: [
                     Text(localeText(context, 'translation'), style: style,),
                     SizedBox(
                       height: 20.h,
                       width: 30.w,
                       child: FittedBox(
                         ...
                         child: CupertinoSwitch(
                             ...
                             activeColor: value.mainBrandingColor,
                             onChanged: (value)=>font.setIsTranslationText(value)),
                       ),
                     ),
                   ],
                 ),
                 ...
                 Row(
                   ...
                   children: [
                     SizedBox(
                         ...
                         child: Text(localeText(context, 'translation_language'), style: style,)),
                     DropdownButton<int>(
                 ...
                 InkWell(
                   onTap: () {
                     context.read<FontProvider>().setQuranSettings(font.fontSizeAr.toInt(), font.fontSizeTrans.toInt(),font.isQuranShow,font.isTranShow);
                     transProvider.saveSelectedTranslation(context);
                     if(_quranProvider.surahId != -1){
                       _quranProvider.setSurahText(surahId: _quranProvider.surahId!, title: _quranProvider.title!, fromWhere: _quranProvider.fromWhere!);
                     }else{
                       _quranProvider.setJuzText(juzId: _quranProvider.juzId!, title: _quranProvider.title!, fromWhere: _quranProvider.fromWhere!,isJuz: true,);
                     }
                     Navigator.of(context).pop();
                   },
                   child: Container(
