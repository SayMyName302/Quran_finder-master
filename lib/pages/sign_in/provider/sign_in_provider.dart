          ...
          await FirebaseAuth.instance.signInWithCredential(credential).then((userCredential) async {
            ...
            if (usersList.isNotEmpty) {
              ...
              if (userIndex != -1) {
                ...
                UserProfile userProfile = await _setUserProfile(userCredential: userCredential, loginType: "google",);
          ...
          await FirebaseAuth.instance.signInWithCredential(authCredential).then((userCredential) async {
            ...
            if (usersList.isNotEmpty) {
              ...
              if (userIndex != -1) {
                ...
                UserProfile userProfile = await _setUserProfile(userCredential: userCredential, loginType: "facebook", image: data["picture"]['data']['url']);
                Future.delayed(Duration.zero, () => context.read<ProfileProvider>().uploadDataToFireStore(userCredential.user!.uid, userProfile));
              }
            } else {
  ...
  signUpWithEmailPassword(String email, String password, String name, BuildContext context) async {
    try {
      ...
      await FirebaseAuth.instance.createUserWithEmailAndPassword(email: email, password: password).then((userCredential) async {
        UserProfile userProfile = await _setUserProfile(email: email, password: password, name: name, userCredential: userCredential, loginType: "email");
