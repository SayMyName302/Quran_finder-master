...
class SignInProvider extends ChangeNotifier{
    ...
    try{
      ...
      if(googleUser != null){
        ...
        try{
          await FirebaseAuth.instance.signInWithCredential(credential).then((userCredential) async{
            ...
            if(usersList.isNotEmpty){
              ...
              if(userIndex != -1){
                ...
                UserProfile userProfile = await setUserProfile(userCredential: userCredential, loginType: "google",);
          ...
          Future.delayed(Duration.zero,()=>EasyLoadingDialog.dismiss(RouteHelper.currentContext));
  ...
  signInWithFaceBook() async {
    try{
      ...
      LoginResult facebookAuth = await FacebookAuth.instance.login(permissions: ["email", "public_profile", "user_friends"]);
      if(facebookAuth.accessToken != null){
        ...
        try{
          ...
          await FirebaseAuth.instance.signInWithCredential(authCredential).then((userCredential) async{
            /// to check weather user exist in the existing list of fire store database
            var doc = await FirebaseFirestore.instance.collection("users").get();
            List<UserProfile> usersList = doc.docs.map((e) => UserProfile.fromJson(e.data())).toList();
            /// if list is not empty means there are some users logged in to this app
            if(usersList.isNotEmpty){
              ...
              if(userIndex != -1){
                ...
                UserProfile userProfile = await setUserProfile(userCredential: userCredential, loginType: "facebook",image: data["picture"]['data']['url']);
                Future.delayed(Duration.zero,()=>RouteHelper.currentContext.read<ProfileProvider>().uploadDataToFireStore(userCredential.user!.uid, userProfile));
              }
            }else{
              UserProfile userProfile = await setUserProfile(userCredential: userCredential, loginType: "facebook",image: data["picture"]['data']['url']);
  ...
  signInWithEmailPassword(String email,String password) async {
    try{
      Future.delayed(Duration.zero,()=>EasyLoadingDialog.show(context: RouteHelper.currentContext,radius: 20.r));
      await FirebaseAuth.instance.signInWithEmailAndPassword(email: email, password: password).then((userCredential) async{
        /// to check weather user exist in the existing list of fire store database
        var doc = await FirebaseFirestore.instance.collection("users").get();
        List<UserProfile> usersList = doc.docs.map((e) => UserProfile.fromJson(e.data())).toList();
        if(usersList.isNotEmpty){
  ...
  signUpWithEmailPassword(String email,String password,String name) async {
    try{
      Future.delayed(Duration.zero,()=>EasyLoadingDialog.show(context: RouteHelper.currentContext,radius: 20.r));
      await FirebaseAuth.instance.createUserWithEmailAndPassword(email: email, password: password).then((userCredential) async{
        UserProfile userProfile = await setUserProfile(email: email, password: password, name: name, userCredential: userCredential, loginType: "email");
        Future.delayed(Duration.zero, ()=>Provider.of<ProfileProvider>(RouteHelper.currentContext,listen: false).uploadDataToFireStore(userCredential.user!.uid, userProfile));
      });
    }on FirebaseAuthException catch (e){
      showErrorSnackBar(e.message.toString());
      EasyLoadingDialog.dismiss(RouteHelper.currentContext);
  ...
  showErrorSnackBar(String msg){
    ScaffoldMessenger.of(RouteHelper.currentContext).showSnackBar(SnackBar(content: Text(msg)));
  }
